...
public class GeneticAlgorithm {
	...
	SpeciesIndividual run(SpeciesPopulation list)
	{
		...
		for(int i=1;i<=TSPData.DEVELOP_NUM;i++)//进化代数
		{
			//选择
			select(list);
	...
	{
		//100%随机
		int randomNum=(int)(TSPData.SPECIES_NUM);//种群中物种的数量
		for(int i=1;i<=randomNum;i++)
		{
			SpeciesIndividual species=new SpeciesIndividual();//创建结点(个体类)
			species.createByRandomGenes();//初始种群基因(基因数量是城市的总数量,比如30个基因,最终随机成{20,1,30,8,28'''''})
